...
import { ReporterForm, Page,  PagedData } from '../../models';
import { APIService } from '../api/api.service';

@Injectable()
export class ReporterFormService {

  private className = 'ReporterForm';
   ...
   */
  search(page: Page): Promise<PagedData<ReporterForm>> {
    const pagedData = new PagedData<ReporterForm>();
    return new Promise(async (resolve, reject) => {
      try {
        ...
        subscription.on('create', (data) => {
          ...
          const form = new ReporterForm(data.toJSON());
        ...
        results.forEach((reporterform) => {
          const form = new ReporterForm(reporterform.toJSON());
          pagedData.data.push(form);
        })
